...
class ByteLevelTransformerBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        # build the ffn norm
        self.ffn_norm = build_normalization(
            normalization_name="rms_norm", #ffn_cfg.get("normalization", "none"), # Default: none
            dim=input_dim,
            bias=False, #ffn_cfg["params"]["bias"],
        )
        ...
        self.res_proj_layer = torch.nn.Linear(
            input_dim,
            output_dim,
            bias=False
        )
    ...
    def forward(self, x, attn_mask=None):
        ...
        x = self.res_proj_layer(x) + self.ffn(self.ffn_norm(x)) # TODO find a better solution for this
        return x
