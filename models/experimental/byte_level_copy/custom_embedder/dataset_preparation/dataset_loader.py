...
class EndByteClassifierDataset(Dataset):
    ...
    def __init__(self, x_path: str, y_path: str, config: dict):
        ...
        self.dataset_len = len(self.x_data) - self.context_window + 1
    ...
    def __getitem__(self, idx):
        x = torch.tensor(self.x_data[idx : idx + self.context_window], dtype=torch.long, device=self.device)
        y = torch.tensor(self.y_data[idx : idx + self.context_window], dtype=torch.float32, device=self.device)
        return x, y
